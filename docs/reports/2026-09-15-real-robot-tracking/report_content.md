...
表 3 汇总了 Artstein-HPC 实验二参考组与 λ初值为 2.5 的 Artstein-LPC 实物记录。两组 Leader 速度接近 0.25 m/s，结果用于说明当前阶段的跟踪现象。

LPC 在 λ初值为 1.5、Leader 速度为 0.25 m/s 的实物工况下发生碰撞，未形成可用于轨迹统计的有效记录。λ初值为 2.5、Leader 速度为 0.25 m/s 时可完成有效跟踪记录。因此现有 HPC/LPC 结果用于说明阶段性可运行性和现象，不用于宣称严格单变量条件下的性能优劣。
