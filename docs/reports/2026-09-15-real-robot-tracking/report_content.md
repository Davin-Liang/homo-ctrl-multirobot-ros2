...
动捕系统分别提供 Leader 与 Follower 在统一坐标系下的位置和速度状态。控制器根据两车相对状态及期望编队半径构造跟踪误差，并从编队圆上的离散候选点中选择当前跟踪目标；在目标切换过程中设置迟滞条件，以避免两台机器人相对位置接近分界处时频繁切换目标点。
...
控制律计算得到的平面速度指令会转换到 Follower 车体系，并依次经过最大线速度、最大加速度和轮速限制处理后发布。这样既保证控制器按预测状态进行跟踪，又使最终命令满足全向底盘的实际执行能力。
...
实物实验平台由两台构型相同的全向移动机器人组成。每台机器人配备全向底盘、激光测距传感器、控制与通信组件，可在室内场地内完成平面任意方向运动。实验过程中，两台机器人由动捕系统提供统一坐标系下的状态反馈，并通过无线通信完成控制与状态信息交互。
...
四组有效记录均在同一实物平台和动捕系统下采集。Leader 与 Follower 分别运行在 `/robot1` 和 `/robot2` 命名空间下，动捕以 120 Hz 提供状态反馈，控制器以 20 Hz 输出速度命令。实验以 1.0 m 为期望编队半径，后续距离误差统计均相对于该半径计算；每组记录保存两车的位置、速度及相对距离等时序数据，并同步保存参数快照，用于图表绘制和结果对比。
...
表中“平均距离误差”和“末 10 s 距离误差”均为平均绝对距离误差，“末 10 s 距离误差标准差”反映跟踪后段的距离波动。相对速度误差定义为 Leader 与 Follower 在平面内速度分量之差的模长；数值越小，表示 Follower 的实际运动速度与 Leader 越一致。

| 工况 | 平均距离误差 (m) | RMS 距离误差 (m) | 末 10 s 距离误差 (m) | 末 10 s 距离误差标准差 (m) | 平均相对速度误差 (m/s) | 末 10 s 相对速度误差 (m/s) |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
总体来看，在本次实物记录中，开启 Leader 命令速度前馈后，Follower 在全过程和跟踪后段均表现出更小的距离偏差、速度偏差与波动。该结果说明前馈机制对 Leader 速度变化具有一定的提前响应作用。
