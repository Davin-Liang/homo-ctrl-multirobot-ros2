...
为避免单个结束时刻对结果的偶然影响，表 2 同时统计末 10 s 的平均绝对距离误差和距离误差标准差，用于反映跟踪后段的偏差与波动。前馈开启组的末 10 s 平均绝对距离误差为 0.0444 m、误差标准差为 0.0471 m；关闭组分别为 0.0485 m 和 0.0515 m。因此该组对比仅描述已记录的实测差异，不将其概括为所有指标的一致改善。
