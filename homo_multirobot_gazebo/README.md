...
本包负责 **仿真世界、Gazebo 进程、spawn、ROS 侧 robot/joint 状态、可选 RViz 与 world 静态 TF**；**里程计与平面/全向运动控制** 等按路线图在 URDF 或后续控制包中扩展（见「下一步计划」）。
...
| `publish_world_tf` | `true` | 发布 `world -> <prefix>base_footprint` 静态 TF，便于 RViz **Fixed Frame = world** 同时显示两车；若已有 Gazebo/里程计发布同名 TF，请设 `false` |
...
在 **`homo_multirobot_urdf`** 的 xacro 中已配置 Gazebo 插件（`use_gazebo:=true`），并通过 launch 传入 `ros_namespace:=/robot1` 等，多机话题形如：
...
- **已有**：双机 spawn、空世界、命名空间隔离、`robot_description` / `joint_states`、Gazebo 激光与 IMU 话题、可选 RViz 与 world 静态 TF（仅初始位姿对齐）。
- **尚未实现（建议作为下一步）**：`/<ns>/cmd_vel` 驱动、`/<ns>/odom`、Gazebo 与 RViz 随动的 **动态** `world`/`odom`→`base` TF；全向轮 **接触/力矩** 与 **ros2_control** 等。
...
| 优先级 | 内容 | 建议落点 |
|--------|------|----------|
| 高 | 平面运动：`cmd_vel` → 底盘运动，并发布 `odom` 与 TF | `homo_multirobot_urdf` 中 `gazebo_ros_planar_move`（或等价插件），话题与 `robot{1,2}_namespace` 一致 |
