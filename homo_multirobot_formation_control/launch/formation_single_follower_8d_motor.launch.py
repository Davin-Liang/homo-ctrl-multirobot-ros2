...
def generate_launch_description():
    ...
    # Motor delay simulation (给 Gazebo 注死区，匹配 Pade 模型)
    ...
    formation_node = Node(
        ...
        parameters=[{
            ...
            "hpc_nu": LaunchConfiguration("hpc_nu"),
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument("hpc_c_min", default_value="0.95",
                              description="HPC warp clamp. Lower = more warp, "
                                          "higher = closer to LPC. 8D default 0.95."),
        DeclareLaunchArgument("hpc_nu", default_value="0.3",
                              description="HPC homogeneity degree. Larger = stronger "
                                          "warp (position amplified more). Scan 0.1-0.5."),
        DeclareLaunchArgument("leader_vel_lpf_tau", default_value="0.0",
                              description="Leader velocity low-pass filter time "
                                          "constant (s). 0.0 = disabled."),
        ...
        DeclareLaunchArgument("use_motor_delay", default_value="false",
                              description="Enable simulated motor delay injection "
                                          "(LP + transport + accel limit). Set to "
                                          "true for simulation to match Pade model's "
                                          "Td parameter—Gazebo has no real dead-time."),
        DeclareLaunchArgument("motor_tau", default_value="0.43",
                              description="Simulated motor LP time constant (s). "
                                          "Set to match controller's tau."),
        DeclareLaunchArgument("transport_delay", default_value="0.22",
                              description="Simulated transport delay (s). "
                                          "Set to match controller's Td so the Pade "
                                          "model has a delay to compensate."),
