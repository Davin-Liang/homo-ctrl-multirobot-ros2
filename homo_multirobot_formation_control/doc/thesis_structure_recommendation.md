...
### 第 5 章 基于 predictor-HOCBF 的延迟感知安全编队控制

主要内容：

- 静态环境障碍物下的延迟编队安全问题；
- 6D Artstein Disc 名义编队命令；
- map 系一阶执行器、输入死区与预测状态；
- 静态圆障碍物的二阶 HOCBF；
- 多障碍物硬约束 QP 与输入可行集；
- 物理安全半径与内部过滤半径；
- 连续 predictor-HOCBF 理论、20 Hz 零阶保持与 6D body/map 耦合残差边界；
- 数值仿真与 Gazebo 验证。

本章核心贡献：

```text
在 6D Artstein Disc 名义编队命令的基础上，构造预测状态上的二阶控制障碍函数，
并通过硬约束 QP 生成最小修正安全命令。该方法在已知常值输入死区、一阶执行器模型、
静态障碍物和输入集可行条件下具有连续时间 predictor-HOCBF 安全理论基础；20 Hz
实现以内部过滤半径和高频数值参考评估采样工程边界。
```

建议重点展示：

- 无避障 6D Artstein Disc、旧 OA 软惩罚层、predictor-HOCBF 的对比；
- 普通 HOCBF 与 predictor-HOCBF 的延迟消融对比；
- 单圆柱、多静态圆柱和 QP 可行/不可行情形；
- 物理最小距离、预测 h、命令修正量、QP 不可行次数和编队恢复误差；
- 物理安全半径与过滤半径扫描；
- 20 Hz 与高频参考的对照。

本章理论边界：连续时间结论严格针对 map 系近似一阶执行器和静态障碍物模型；完整
6D body/map yaw 耦合、20 Hz 零阶保持、轮速后处理和感知误差通过预测残差、内部
过滤半径和数值实验评估，不宣称完整离散系统或未知参数下的严格安全证明。
...
3. **MPC**
   ...
   MPC 是约束处理能力强的代表性方法，但计算量和模型依赖更高。本文不将 MPC
   作为实现对照或主章节内容。
