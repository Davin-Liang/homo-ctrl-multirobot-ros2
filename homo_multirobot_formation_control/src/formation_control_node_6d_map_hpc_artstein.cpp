...
: rclcpp::Node("formation_control_node_6d_map_hpc_artstein")
{
  ...
  follower_sub_ = create_subscription<nav_msgs::msg::Odometry>(
    ...
    leader_mocap_pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(leader_ns_ + "/mocap/pose", qos, [this](geometry_msgs::msg::PoseStamped::SharedPtr m) { leader_mocap_pose_ = m; leader_mocap_received_ = now(); leader_ok_ = true; });
...
void FormationController6DMapHpcArtstein::timer_cb()
{
  ...
  if (state_source_ == "ekf_tf") {
    ...
    auto fill = [this](const auto& pose, const auto& twist, State6D& state) {
      ...
      state.x << pose->pose.position.x, pose->pose.position.y, yaw, v_body(0), v_body(1), twist->twist.angular.z;
      state.v_map = v_map;
    };
    fill(leader_mocap_pose_, leader_mocap_twist_, leader);
