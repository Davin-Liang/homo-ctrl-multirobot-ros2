...
FormationController6DArtsteinDisc::FormationController6DArtsteinDisc()
: rclcpp::Node("formation_control_node_6d_artstein_disc")
{
  ...
  double hpc_yaw_threshold = declare_parameter("hpc_yaw_threshold", 0.3);
  double stability_margin = declare_parameter("stability_margin", 0.01);
  ...
  ctrl_ = std::make_unique<LpcController6DArtsteinDisc>(
      radius, mass, inertia, m_p, tol, use_hpc, dt, hpc_c_min,
      initial_min_lambda, switch_min_lambda, hpc_vel_threshold,
      hpc_yaw_threshold, stability_margin);
...
void FormationController6DArtsteinDisc::build_predictors(double tau_v,
                                                         double tau_w,
                                                         double Td,
                                                         double dt)
{
...
bool FormationController6DArtsteinDisc::odom_to_state(
    const std::string& ns,
...
Eigen::VectorXd FormationController6DArtsteinDisc::predict_leader_state(
...
Eigen::VectorXd FormationController6DArtsteinDisc::predict_follower_state(
...
void FormationController6DArtsteinDisc::timer_cb()
{
  ...
  if (!controller_initialized_) {
    ...
    try {
      ctrl_->controller_initial(x1_h, x2_h);
      controller_initialized_ = true;
      RCLCPP_INFO(get_logger(), "6D Artstein Disc controller initialized.");
...
double FormationController6DArtsteinDisc::wrap_angle(double a)
{
  return std::atan2(std::sin(a), std::cos(a));
}

Eigen::Vector2d FormationController6DArtsteinDisc::body_to_map(
...
Eigen::Vector2d FormationController6DArtsteinDisc::map_to_body(
