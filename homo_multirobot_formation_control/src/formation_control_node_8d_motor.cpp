...
// ============================================================================
// 将 EKF 里程计变换到 map 坐标系（照搬 6D Motor，一字不改）
// ============================================================================
...
: rclcpp::Node("formation_control_node_8d_motor")
{
  ...
  double hpc_nu     = declare_parameter("hpc_nu",     0.3);
  ...
  ctrl_ = std::make_unique<LpcController8DMotor>(
      ...
      omega_d, use_hpc, 1.0 / control_rate_, hpc_c_min, hpc_nu,
      tau_min, tau_max, v_tau_trans);
