...
// 构造函数
// ============================================================================
FormationController6DDisc::FormationController6DDisc()
: rclcpp::Node("formation_control_node_6d_disc")
{
  ...
  ctrl_ = std::make_unique<LpcController6DDisc>(radius, mass, I,
  ...
  RCLCPP_INFO(get_logger(), "6D Disc 编队控制节点已启动。");
  RCLCPP_INFO(get_logger(), "  领航者: %s, 跟随者: %s, 安全半径: %.1f m, m_p=%d tol=%.2f",
    leader_ns_.c_str(), follower_ns_.c_str(), radius, m_p, tol);
}

// ============================================================================
// 定时器回调 — 主控制循环 (~20 Hz)
// ============================================================================
void FormationController6DDisc::timer_cb()
{
  ...
  if (!controller_initialized_) {
    try {
      ...
      RCLCPP_INFO(get_logger(), "6D Disc 控制器初始化完成。");
