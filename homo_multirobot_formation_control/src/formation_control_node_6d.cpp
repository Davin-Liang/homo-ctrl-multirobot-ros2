...
//   - px, py, θ: map 系（通过 map→odom TF 变换）
//   - vx_body, vy_body, ω: 车体系（直接取自 EKF，不旋转）
// ============================================================================
...
// 构造函数
// ============================================================================
FormationController6D::FormationController6D()
: rclcpp::Node("formation_control_node_6d")
{
  ...
  RCLCPP_INFO(get_logger(), "6D 编队控制节点已启动。");
...
// 定时器回调 — 主控制循环 (~20 Hz)
// ============================================================================
void FormationController6D::timer_cb()
{
  ...
  if (!controller_initialized_) {
    try {
      ...
      RCLCPP_INFO(get_logger(), "6D 控制器初始化完成。");
