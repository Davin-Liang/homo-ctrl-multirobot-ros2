...
/// @file 4D Artstein-MPC 编队控制 ROS 2 节点 — TF + EKF 数据管线。
///
...
class FormationController4DArtsteinMpc : public rclcpp::Node
{
public:
  FormationController4DArtsteinMpc();
...
private:
  ...
  std::unique_ptr<formation_control::MpcController4DArtstein> mpc_;
  ...
  int mpc_solved_count_ = 0;
  int mpc_inaccurate_count_ = 0;
  int mpc_max_iter_count_ = 0;
  int mpc_fallback_count_ = 0;
