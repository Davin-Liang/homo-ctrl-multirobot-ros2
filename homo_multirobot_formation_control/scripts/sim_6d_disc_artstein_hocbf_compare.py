...
def filter_translation_command(
    ...
    result = solve_hocbf_qp(
        cmd_nom_map, previous_map_command, [(a, b)], vmax, amax, dt
    )
...
def simulate_compensated_hocbf(Tmax, h, tau_v, tau_w, Td, obstacle, safe_radius,
                               leader_heading_fixed=False):
    ...
    while t < Tmax - 1e-12:
        ...
        safe, safe_map, feasible, hval = filter_translation_command(
            x2c, yaw_meas, nominal, obstacle, safe_radius, tau_v, 2.0, 2.0,
            prev_map, 1.0, 20.0, h)
...
def main():
    ...
    if args.auto_obstacle_offset is not None:
        i = len(base) // 2
        point = x2[:2, i]
    ...
    safe = simulate_compensated_hocbf(args.tmax, .05, .43, .43, .22, obstacle,
                                      radius, args.leader_heading_fixed)
    ...
    ax[0].add_patch(plt.Circle(obstacle, args.physical_radius, fill=False, color="r", linewidth=2, label="physical safety"))
    ax[0].add_patch(plt.Circle(obstacle, radius, fill=False, color="orange", linestyle="--", linewidth=2, label="HOCBF filter"))
