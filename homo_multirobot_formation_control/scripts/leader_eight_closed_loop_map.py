#!/usr/bin/env python3
"""Delay-aware map-frame closed-loop figure-eight generator for a Leader.

The node holds a fixed map-frame heading while tracking a circular position
reference from EKF/odometry or motion-capture feedback. It predicts the
translational state over the configured command dead time and response horizon.
"""
...
class LeaderEightClosedLoopMap(Node):
    def __init__(self) -> None:
        super().__init__('leader_eight_closed_loop_map')
        ...
        if (self.amplitude_x <= 0.0 or self.amplitude_y <= 0.0 or
                self.speed < 0.0 or self.rate <= 0.0):
            raise ValueError(
                'amplitudes and rate must be positive; speed must be non-negative')
