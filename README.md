...
| **`homo_multirobot_formation_control`** | Leader-Follower 编队控制（齐次控制算法，C++ / Eigen）。包含 4D、4D Artstein、4D Artstein-LQR、4D Cont、6D、6D Disc、6D Motor、旧 6D+OA，以及 **6D Artstein Disc + predictor-HOCBF**。HOCBF 版本仅从 `/scan` 拟合静态圆柱，在 map 系预测状态上施加多圆柱硬 QP 安全约束。 |
...
```
> 该旧 OA 为软惩罚层；新的 HOCBF 版本见下方。

**6D Artstein Disc + predictor-HOCBF**（scan 拟合静态圆柱 + map 系多圆柱硬 QP）：
```bash
...
```
> 不传障碍物真值位置/半径；当前仅支持 scan 可见静态圆柱。
...
数学推导：`homo_multirobot_formation_control/doc/kinematic_homogeneous_control.md`（论文版：`doc/kinematic_homogeneous_control_full.md`）
